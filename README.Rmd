...
`"coords_*"` (default — clusters on the (x, y) path coordinates, so paths
that look alike in the plot cluster together), `"heading_*"` (cumulative
heading sequence, rotation-invariant), and `"area_*"` (shoelace area between
...
Pass the cluster column to `cluster_hull` to draw colour-matched concave hulls
around each cluster's locations. Hull fill colour automatically matches the
cluster's `flower_colors`. Use `hull_coverage` (default `1`) to focus the hull
on the core of each cluster and reduce overlap when clusters are dense:

```{r cluster-hull, fig.width = 12, fig.height = 6, eval = requireNamespace("patchwork", quietly = TRUE) && requireNamespace("maps", quietly = TRUE)}
gw_coords <- dplyr::mutate(gw_long,
  lon = rep(c(6.9, 13.4, 9.9, 12.1, 7.5, 11.2, 8.7, 14.8), each = n),
  lat = rep(c(53.6, 52.5, 51.5, 48.1, 51.2, 49.8, 47.8, 50.9), each = n)
)
